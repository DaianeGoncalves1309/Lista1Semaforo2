...
public class ThreadCozinha extends Thread{
	...
	private void Cozinha(){
		...
		System.out.println("A "+prato+" "+getId()+" come�ou a ser cozinhada!");
		for(int i=0;i<tempoTotal;i++){
			porcentagem = porcentagem + 100/tempoTotal;
			System.out.println("A "+prato+" "+getId()+" est� cozinhando: "+porcentagem+"%");
			try {
				sleep(1000);
			} catch (InterruptedException e) {
				e.printStackTrace();
		...
		System.out.println("A "+prato+" "+getId()+" est� pronta!");
	}
